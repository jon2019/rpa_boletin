"""
Diagnóstico visible con Selenium para https://www.miningweekly.com

Objetivo:
- abrir Chrome visible para observar el challenge anti-bot
- esperar a que Cloudflare libere el acceso al home internacional
- capturar el HTML renderizado real y probar selectores de noticias
- comparar el resultado con la extracción genérica del scraper productivo

Ejecución desde la raíz del proyecto:
    $env:PYTHONPATH = "$PWD\\src"
    .\\.venv\\Scripts\\python tests\\helper\\integration\\test_miningweekly_selenium_visible.py

Salidas:
- tests/output/integration/test_miningweekly_selenium_visible.log
- tests/output/integration/test_miningweekly_selenium_visible_resultado.json
"""
...
log = logging.getLogger("test_miningweekly_selenium_visible")
...
RESULT_JSON = RESULT_DIR / "test_miningweekly_selenium_visible_resultado.json"
...
def setup_logging() -> Path:
    ...
    log_file = RESULT_DIR / "test_miningweekly_selenium_visible.log"
...
def _extraer_categoria_desde_container(container) -> str:
    ...
    categoria_node = container.select_one(".btn-news, .cm-sponsored-post, .category, .categoria")
    if categoria_node:
    ...
    for selector in SELECTORES_CANDIDATOS:
        for anchor in soup.select(selector):
            ...
            container = anchor.find_parent(
                ...
                and any(token in " ".join(value).lower() for token in ("card", "entry"))
            )
...
def _comparar_con_scraper_generico(soup: BeautifulSoup, base_url: str) -> dict:
    try:
        noticias = extract_noticias_from_soup(
            SOURCE_FIXTURE,
            soup,
            base_url,
            "Test Mining Weekly Selenium Visible",
        )
    except Exception as exc:
        log.exception("Fall? la comparaci?n con extract_noticias_from_soup: %s", exc)
        return {
            "total": 0,
            "preview": [{"titulo": "ERROR", "url": str(exc)}],
        }
...
def _esperar_contenido_real(
    ...
    while time.time() - inicio < timeout_segundos:
        ...
        if challenge_activo:
            ...
            log.info(
                "Sin challenge aparente, pero todavía sin selectores de noticias | title=%s",
                current_title,
            )
...
def _descargar_html_con_selenium_visible(
    timeout_segundos: int = 60,
) -> tuple[str, str, dict]:
    """
    Levanta Chrome visible para observar el challenge y devuelve el HTML final.
    """
    options = Options()
    ...
    try:
        ...
        challenge_resuelto, detalle = _esperar_contenido_real(
            driver,
        ...
        log.info(
            "Selenium visible | URL final: %s | título: %s | html_len=%d | cookies=%d | estado=%s",
            final_url,
            driver.title,
            len(html),
...
def run_test():
    ...
    resultado = {
        ...
        "url": TARGET_URL,
        "url_final": None,
        "content_type": "text/html; browser-rendered-visible",
        "selectores": [],
    ...
    try:
        html, final_url, browser_meta = _descargar_html_con_selenium_visible(timeout_segundos=60)
        resultado["url_final"] = final_url
        resultado["browser"] = browser_meta

        if not html:
            raise RuntimeError(browser_meta.get("estado_browser") or "Selenium visible no devolvió HTML")
        ...
        resultado["scraper_generico_encontradas"] = scraper_preview["total"]
        resultado["scraper_generico_preview"] = scraper_preview["preview"]
        resultado["estado"] = "ok" if browser_meta.get("challenge_resuelto") else "cloudflare_blocked"

        log.info(
            "Resumen test Mining Weekly Selenium visible | noticias_cards=%d | scraper_generico=%d | estado=%s",
            len(noticias),
            scraper_preview["total"],
            resultado["estado"],
        )
    except Exception as exc:
        resultado["estado"] = "error"
        resultado["error"] = str(exc)
        log.exception("Error ejecutando test Selenium visible de Mining Weekly: %s", exc)
    finally:
        resultado["timestamp_fin"] = time.strftime("%Y-%m-%d %H:%M:%S")
        _guardar_resultado(resultado)

    log.info("=== FIN TEST MINING WEEKLY SELENIUM VISIBLE ===")
    return resultado
